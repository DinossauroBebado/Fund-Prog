//Modifique o exercício 4 para restringir a agenda ao mês de setembro de 2018 e incluir a informação
//do dia da semana (“Domingo”, “Segunda-Feira”, etc.). O registro desta informação deve ocorrer com
//uso coerente de enumeração. A estrutura de dados deve prever um campo para dia da semana na
//forma de inteiros, e a enumeração deve ser usada para impressão da cadeia de caracteres referente
//ao dia da semana.
//
...
#include <conio.h>
 ...
 enum  dia_da_semana {Sabado,Domingo,Segunda_feira,Terca_feira,
...
int main()
{
    ...
    char semana[][20] = { "Sabado","Domingo","Segunda_feira","Terca_feira",
             "Quarta_feira","Quinta_feira","Sexta_feira"
              }; //char com as posições equivalente ao enum dia_da_semana
     ...
     for(int i = 0;i<n;i++){
      ...
      for(int j = 0; j<n;j++){
       if(agenda[j].ano==2018 && agenda[j].mes==9){
        ...
        printf("%d/%d/%d -%s |",agenda[j].dia,agenda[j].mes,agenda[j].ano,semana[agenda[j].sem - 1]);printf("%.1d :%.1d |",agenda[j].hora,agenda[j].minuto);
